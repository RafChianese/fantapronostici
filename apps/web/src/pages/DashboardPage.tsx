  ...
  const Orb = ({ p }: { p: { md: number; pts: number; tone: string; status: string } }) => {
    ...
    return (
      <Link
        ...
        className="group relative grid h-[48px] w-[48px] sm:h-[68px] sm:w-[68px] place-items-center rounded-full border border-emerald-100/15 bg-emerald-100/5 shadow-[0_10px_30px_rgba(0,0,0,0.35)] hover:bg-emerald-100/10"
      ...
      >
        ...
        <div className="relative z-[1] text-center">
          <div className="text-[10px] sm:text-[11px] font-bold text-emerald-50/70">G{p.md}</div>
  ...
  const MatchOrb = ({ p }: { p: { id: string; md: number; home: string; away: string; status: string; total: number; tone: string } }) => {
    ...
    return (
      <Link
        ...
        className="group relative grid h-[58px] w-[58px] shrink-0 place-items-center rounded-full border border-emerald-100/15 bg-emerald-100/5 transition hover:-translate-y-0.5 hover:bg-emerald-100/10 sm:h-[70px] sm:w-[70px]"
      ...
      >
        ...
        <div className="relative z-[1] text-center leading-none">
          <div className="text-[9px] font-black tracking-tight text-emerald-50/85 sm:text-[10px]">{home}</div>
          <div className={`my-1 text-[13px] font-extrabold sm:text-[15px] ${p.status === "IN_PROGRESS" ? "text-blue-100" : "text-white"}`}>{label}</div>
          <div className="text-[9px] font-black tracking-tight text-emerald-50/85 sm:text-[10px]">{away}</div>
        </div>
      </Link>
    );
  };
      ...
      <div className="mx-auto flex min-h-[55vh] w-full max-w-2xl items-center justify-center px-4">
        <div className="tm-glass flex flex-col items-center gap-4 rounded-3xl px-8 py-7 text-center">
          ...
          <div>
            ...
            <div className="mt-1 text-xs text-emerald-50/70">Sto recuperando classifica, pronostici e partite.</div>
          </div>
        </div>
      </div>
    );
    ...
    <div className="mx-auto w-full max-w-2xl space-y-4">
      {approved.length > 1 ? (
        <Card>
          ...
          <CardContent>
            <select
              className="w-full rounded-xl border border-emerald-100/15 bg-emerald-950/35 px-3 py-3 text-sm font-semibold text-white outline-none focus:ring-2 focus:ring-rose-500/30"
          ...
          <div className="p-4 sm:p-5">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="text-xs font-bold uppercase tracking-wide text-emerald-50/70">{isTournamentPreMode ? "Pronostici partite" : "Prossima giornata"}</div>
              ...
              <div className="shrink-0 text-right">
                <div className="text-xs text-emerald-50/70">Countdown</div>
                <div className="mt-1 inline-flex items-center rounded-xl border border-emerald-100/15 bg-emerald-100/5 px-3 py-2 text-sm font-extrabold">
                  {nextCountdown || "—"}
                </div>
              </div>
            </div>
              ...
              <>
                <div className="mt-4 flex items-center justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <div className="rounded-full bg-emerald-100/10 p-1">
                      <div className="h-2.5 rounded-full bg-rose-500/80 transition-[width]" style={{ width: `${nextPct}%` }} />
                    </div>
                    <div className="mt-2 text-xs text-emerald-50/70">
...
<div className="flex items-center gap-2">
  <div
    className="relative grid h-[74px] w-[74px] shrink-0 place-items-center rounded-full border border-emerald-100/15 bg-emerald-100/5 shadow-[0_10px_30px_rgba(0,0,0,0.35)]"
  ...
  >
    <div className="grid h-[58px] w-[58px] place-items-center rounded-full border border-emerald-100/15 bg-slate-950/50">
      <div className="text-center">
        <div className="text-base font-extrabold text-white">
          {nextDone}/{nextTotal || "—"}
        </div>
        <div className="text-[10px] font-semibold text-emerald-100/60">match</div>
      </div>
    </div>
  </div>
    ...
    >
      <div className="grid h-[58px] w-[58px] place-items-center rounded-full border border-emerald-100/15 bg-slate-950/50">
        <div className="text-center">
          ...
          <div className="text-[10px] font-semibold text-emerald-100/60">torneo</div>
        </div>
      </div>
    </div>
  ) : null}
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex items-center gap-3">
            <div className="shrink-0 rounded-2xl border border-emerald-100/15 bg-emerald-100/5 p-2">
              <UserAvatar avatarId={(user as any)?.avatarId} size={64} mode="full" className="rounded-2xl" />
            </div>
            <div className="grid w-full grid-cols-2 gap-2 sm:grid-cols-4">
              <div className="rounded-2xl border border-emerald-100/15 bg-emerald-100/5 p-3">
                <div className="text-[11px] font-semibold text-emerald-100/60">Posizione</div>
                <div className="mt-0.5 text-lg font-extrabold text-white">{loading ? "—" : myPosition ?? "—"}</div>
              </div>
              <div className="rounded-2xl border border-emerald-100/15 bg-emerald-100/5 p-3">
                <div className="text-[11px] font-semibold text-emerald-100/60">Punti</div>
                <div className="mt-0.5 text-lg font-extrabold text-white">
                  {loading ? "—" : <AnimatedNumber value={Number(totals?.total ?? 0)} />}
                </div>
              </div>
              <div className="rounded-2xl border border-emerald-100/15 bg-emerald-100/5 p-3">
                <div className="text-[11px] font-semibold text-emerald-100/60">Giornate</div>
                <div className="mt-0.5 text-lg font-extrabold text-white">{loading ? "—" : playedMatchdays}</div>
              </div>
              <div className="rounded-2xl border border-emerald-100/15 bg-emerald-100/5 p-3">
                <div className="text-[11px] font-semibold text-emerald-100/60">Esatti</div>
                <div className="mt-0.5 text-lg font-extrabold text-white">
      ...
      <Card>
        ...
        <CardContent>
          {lastPills.length ? (
            ...
            <div className="text-sm text-emerald-50/70">Nessuna giornata disponibile.</div>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          {recentMatchPills.length ? (
            ...
            <div className="text-sm text-emerald-50/70">Nessuna partita giocata o live disponibile.</div>
          )}
          <div className="mt-3 flex flex-wrap items-center gap-2 text-[11px] font-semibold text-emerald-50/70">
            ...
            <span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-emerald-300" /> U/O 2.5</span>
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {recentFinishedMatches.map((m) => (
                <div key={m.id} className="flex items-center justify-between gap-3 rounded-2xl border border-emerald-100/15 bg-emerald-100/5 px-3 py-3">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex min-w-0 items-center gap-2">
                        {m.homeLogo ? (
                          <img src={m.homeLogo} alt="" className="h-7 w-7 rounded-full bg-emerald-100/10 object-contain" />
                        ) : (
                          <div className="grid h-7 w-7 place-items-center rounded-full bg-emerald-100/10 text-xs font-bold text-emerald-50/85">{String(m.homeTeam || "H").slice(0, 1)}</div>
                        )}
                    ...
                    <div className="mt-2 flex items-center justify-between gap-2">
                      <div className="flex min-w-0 items-center gap-2">
                        {m.awayLogo ? (
                          <img src={m.awayLogo} alt="" className="h-7 w-7 rounded-full bg-emerald-100/10 object-contain" />
                        ) : (
                          <div className="grid h-7 w-7 place-items-center rounded-full bg-emerald-100/10 text-xs font-bold text-emerald-50/85">{String(m.awayTeam || "A").slice(0, 1)}</div>
                        )}
                    ...
                    <div className="mt-2 flex items-center justify-between text-xs text-emerald-100/60">
            ...
            <div className="text-sm text-emerald-50/70">Nessun risultato disponibile.</div>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          {loading ? (
              ...
              {top5.map((r, idx) => {
                ...
                return (
                  <Link
                    key={r.userId}
                    to={`/users/${r.userId}`}
                    className={`group block rounded-2xl border px-3 py-2 transition hover:-translate-y-0.5 hover:border-rose-400/40 hover:bg-emerald-100/10 focus:outline-none focus:ring-2 focus:ring-rose-400/40 ${
                      isMe ? "border-rose-400/40 bg-rose-500/10" : "border-emerald-100/15 bg-emerald-100/5"
                    }`}
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex min-w-0 items-center gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="text-[11px] text-emerald-100/60">Tocca per vedere i pronostici</div>
                        </div>
                      </div>
                      <div className="shrink-0 rounded-xl border border-emerald-100/15 bg-black/20 px-2 py-1 text-sm font-extrabold text-white">
                        {r.totalPoints}
                      </div>
                    </div>
                  </Link>
            ...
            <div className="text-sm text-emerald-50/70">Nessun dato classifica.</div>
          )}
        </CardContent>
      </Card>
    </div>
