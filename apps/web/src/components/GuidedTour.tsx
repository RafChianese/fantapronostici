...
export function GuidedTour({
  ...
  return (
    <div className="fixed inset-0 z-[110]" role="dialog" aria-modal="true" aria-label="Tutorial">
      ...
      <div
        className="absolute rounded-2xl bg-emerald-950/45 p-5 shadow-2xl max-h-[70vh] overflow-auto border border-emerald-100/15"
        style={cardStyle}
      >
        <div className="text-xs font-semibold text-emerald-100/60">Passo {idx + 1} / {steps.length}</div>
        <div className="mt-1 text-lg font-semibold text-white">{step.title}</div>
        <div className="mt-2 text-sm text-emerald-50/70 whitespace-pre-line">{step.body}</div>

        <div className="mt-4 flex flex-wrap items-center justify-between gap-2">
          <button
            type="button"
            className="rounded-xl px-3 py-2 text-sm font-medium text-emerald-50/70 hover:bg-slate-900"
            onClick={finish}
          >
            Salta
          </button>
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              className="rounded-xl border border-emerald-200/20 bg-emerald-950/45 px-3 py-2 text-sm font-semibold text-white hover:bg-slate-900 disabled:opacity-50"
