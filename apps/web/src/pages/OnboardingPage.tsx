        ...
        <Card data-tour="join-league">
          ...
          <CardContent>
            <div className="space-y-2">
              {approved.map((m) => (
                <div key={m.id} className="flex items-center justify-between rounded-xl border border-emerald-100/15 bg-emerald-100/5 px-4 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-emerald-100/60">Codice: {m.league.code} • Ruolo: {m.role}</div>
                    </div>
                  </div>
                  <Button
                    onClick={() => {
      ...
      {pending.length > 0 ? (
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              {pending.map((m) => (
                <div key={m.id} className="rounded-xl border border-emerald-100/15 bg-emerald-100/5 px-4 py-3">
                  <div className="font-medium">{m.league.name}</div>
                  <div className="text-xs text-emerald-100/60">Codice: {m.league.code} • Stato: {m.status}</div>
                </div>
              ))}
            </div>
          </CardContent>
