  ...
  return (
    <div className="space-y-2 rounded-xl border border-white/10 bg-slate-950/70 p-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="text-xs text-slate-400">Selezionate: {selected.length}</div>
      </div>
      <Input
        label="Cerca squadra"
        value={query}
      ...
      />
      <div className="max-h-48 overflow-auto rounded-xl border border-white/10 bg-white/5 p-2">
        {options.length ? options.map((team) => (
          <label key={team.id} className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1.5 text-sm text-slate-100 hover:bg-slate-800/70">
            <input
              type="checkbox"
          ...
          <div className="px-2 py-2 text-xs text-slate-400">Nessuna squadra disponibile.</div>
        )}
      </div>
      {selected.length ? (
    ...
    <div className="grid gap-6 md:grid-cols-2">
      ...
      <Card className="md:col-span-2">
        ...
        <CardContent className="space-y-4">
          {!compOutcome ? <div className="text-sm text-slate-400">Caricamento…</div> : null}
          {err ? <Alert tone="danger">{err}</Alert> : null}
            ...
            <>
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <div className="space-y-2 md:col-span-2">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-xs text-slate-400">Usalo solo se vuoi considerare un 2° top scorer (pari merito).</div>
                    </div>
                    {scorer2Id ? (
                ...
                {external?.competitionType === "KNOCKOUT_CUP" ? (
                  <div className="space-y-3 md:col-span-2 rounded-2xl border border-white/10 bg-white/5 p-4">
                    <div>
                      <div className="text-sm font-semibold text-slate-100">Squadre qualificate alle fasi a eliminazione</div>
                      <div className="text-xs text-slate-400">Liste globali usate per assegnare i punti ai pronostici del torneo. Puoi selezionare più squadre per ogni fase.</div>
                    </div>
                    <MultiTeamSelect
              ...
              <div className="flex items-center gap-2">
                ...
                {compOutcome?.outcome?.resolvedAt ? (
                  <span className="text-xs text-slate-400">Ultimo salvataggio: {new Date(compOutcome.outcome.resolvedAt).toLocaleString("it-IT")}</span>
                ) : (
                  <span className="text-xs text-slate-400">Non ancora impostato.</span>
                )}
              </div>
            </>
          ) : null}
          ...
          <div className="space-y-2">
            {leagues.map((l) => (
              ...
              >
                ...
                <div className="mt-1 text-xs text-slate-400">Membri: {l._count?.members ?? 0}</div>
              </button>
            ))}
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          {!selected ? (
            <div className="text-sm text-slate-400">Seleziona una lega a sinistra.</div>
          ) : (
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between rounded-xl border border-white/10 p-3">
          <div>
            ...
            <div className="text-xs text-slate-400">Se OFF, i pronostici sono visibili senza sblocco</div>
          </div>
          <input
            type="checkbox"
        ...
        <div className="flex items-center justify-between rounded-xl border border-white/10 p-3">
          <div>
            ...
            <div className="text-xs text-slate-400">Se ON, usa il countdown demo quando Ad Manager non è disponibile</div>
          </div>
          <input
            type="checkbox"
        ...
        <div className="rounded-xl border border-white/10 p-3">
          ...
          <div className="mt-2 flex items-center gap-2">
            <input
              ...
              className="w-28 rounded-lg border border-white/10 px-3 py-2"
            />
      ...
      <div className="space-y-3">
        <div className="rounded-xl border border-white/10 p-3">
          ...
          <div className="mt-2 grid grid-cols-3 gap-2 text-sm">
            <div className="rounded-lg bg-white/5 p-2">
              <div className="text-xs text-slate-400">Unlock totali</div>
              <div className="font-semibold">{stats?.totalUnlocks ?? 0}</div>
            </div>
            <div className="rounded-lg bg-white/5 p-2">
              <div className="text-xs text-slate-400">Utenti unici</div>
              <div className="font-semibold">{stats?.uniqueUsers ?? 0}</div>
            </div>
            <div className="rounded-lg bg-white/5 p-2">
              <div className="text-xs text-slate-400">Media minuti</div>
        ...
        <div className="rounded-xl border border-white/10 p-3">
          ...
          <div className="mt-2 space-y-2">
            ...
            {(!stats?.last || stats.last.length === 0) ? <div className="text-sm text-slate-400">Nessun dato.</div> : null}
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="tm-page-stack">
      ...
      <div className="flex flex-wrap items-end gap-2">
        <label className="space-y-1 text-sm">
          <div className="text-xs text-slate-400">Season (opz.)</div>
          <input
            type="number"
            className="w-40 rounded-xl border border-white/10 bg-slate-950/70 px-3 py-2"
            value={season}
        ...
        <label className="space-y-1 text-sm">
          <div className="text-xs text-slate-400">Area (opz.)</div>
          <input className="w-44 rounded-xl border border-white/10 bg-slate-950/70 px-3 py-2" value={area} onChange={(e) => setArea(e.target.value)} placeholder="Italy / Europe" />
        </label>
        <label className="space-y-1 text-sm flex-1 min-w-[200px]">
          <div className="text-xs text-slate-400">Search (opz.)</div>
          <input className="w-full rounded-xl border border-white/10 bg-slate-950/70 px-3 py-2" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Serie A, Champions, Worldcup..." />
        </label>
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {comps.slice(0, 12).map((c: any) => {
            ...
            return (
              <button
                key={code}
                className={`rounded-xl border p-3 text-left hover:bg-white/5 ${chosen?.code === code ? "border-slate-900" : "border-white/10"}`}
                onClick={() => setChosen({ code, name, area: areaName })}
              >
                <div className="font-medium">{name}</div>
                <div className="text-xs text-slate-400">{areaName} · Code: {code}</div>
              </button>
            );
          })}
        </div>
...
function FootballDataSyncPanel() {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-end gap-2">
        <label className="space-y-1 text-sm">
          <div className="text-xs text-slate-400">Matchday (opz.)</div>
          <input className="w-40 rounded-xl border border-white/10 bg-slate-950/70 px-3 py-2" value={matchday} onChange={(e) => setMatchday(e.target.value)} placeholder="es. 1" />
        </label>
        <Button
          disabled={loading}
          onClick={async () => {
      ...
      <div className="text-xs text-slate-400">I risultati vengono sincronizzati automaticamente da football-data.org. La modifica manuale è disabilitata per queste partite.</div>
    </div>
  );
}
    ...
    <div className="tm-page-stack">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <label className="space-y-1 text-sm">
          ...
          <select
            className="w-full rounded-xl border border-white/10 bg-slate-950/70 px-3 py-2"
        ...
        <label className="space-y-1 text-sm">
          <div className="text-xs text-slate-400">League ID (API-Football)</div>
          <input
            type="number"
            className="w-full rounded-xl border border-white/10 bg-slate-950/70 px-3 py-2"
        ...
        <label className="space-y-1 text-sm">
          <div className="text-xs text-slate-400">Season (es. 2025 per 2025/26)</div>
          <input
            type="number"
            className="w-full rounded-xl border border-white/10 bg-slate-950/70 px-3 py-2"
      ...
      <div className="rounded-xl border border-white/10 p-3">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          <input
            className="flex-1 rounded-xl border border-white/10 bg-slate-950/70 px-3 py-2"
            value={search}
        ...
        {searchResults.length ? (
          <div className="mt-3 grid gap-2 md:grid-cols-2">
            {searchResults.slice(0, 6).map((l: any) => (
              ...
              >
                <div className="font-medium">{l.name}</div>
                <div className="text-xs text-slate-400">{l.country} · ID: {l.id}</div>
              </button>
            ))}
          </div>
        ) : null}
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-xl border border-white/10 p-3">
        ...
        <div className="text-xs text-slate-400">Code: {league.code} • ID: {league.id}</div>
      </div>
          ...
          <Card>
            ...
            <CardContent>
              ...
              {rules ? (
                  ...
                  <div className="rounded-xl border border-white/10 p-3">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <div className="text-xs text-slate-400">Bonus punti opzionale</div>
                      </div>
                      <input
                        type="checkbox"
                ...
                <div className="text-sm text-slate-400">Nessuna regola trovata.</div>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              {settings ? (
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between rounded-xl border border-white/10 p-3">
                    <div>
                      ...
                      <div className="text-xs text-slate-400">Blocca tutto immediatamente</div>
                    </div>
                    <input
                      type="checkbox"
                ...
                <div className="text-sm text-slate-400">Nessuna impostazione trovata.</div>
              )}
            </CardContent>
          </Card>
        </div>
    ...
    <div className="rounded-xl border border-white/10 bg-slate-950/70 p-4">
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <div className="text-xs text-slate-400">{new Date(match.kickoffAt).toLocaleString()}</div>
        ...
        <div className="flex flex-wrap items-end gap-2">
          <label className="space-y-1 text-sm">
            ...
            <select className="rounded-xl border border-white/10 bg-slate-950/70 px-3 py-2 text-sm" value={status} onChange={(e) => setStatus(e.target.value)}>
