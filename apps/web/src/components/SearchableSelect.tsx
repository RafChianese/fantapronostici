    ...
    <div ref={rootRef} className="relative" data-searchable-select-root="true">
      <button
        ...
        className={`w-full rounded-xl border bg-slate-950/90 px-3 py-2 text-left text-sm transition ${
          disabled ? "border-emerald-100/15 text-emerald-100/60" : "border-emerald-100/20 text-white hover:bg-slate-900/95"
        }`}
      >
        ...
        ? createPortal(
            <div className="fixed inset-0 z-50" aria-hidden="true">
              ...
              <div
                ...
                className="absolute overflow-hidden rounded-2xl border border-emerald-100/20 bg-slate-950 shadow-2xl ring-1 ring-emerald-200/10"
              ...
              >
                <div className="border-b border-emerald-100/15 p-2">
                  <input
                    value={query}
                    onChange={(e) => setQuery(e.target.value)}
                    autoFocus
                    placeholder="Cerca…"
                    className="w-full rounded-xl border border-emerald-100/20 bg-slate-900 px-3 py-2 text-sm text-white placeholder:text-emerald-100/60 outline-none focus:ring-2 focus:ring-emerald-300/35"
                  />
                </div>
                ...
                <div className="overflow-auto p-1" style={{ maxHeight: panel.maxHeight }}>
                  ...
                  {filtered.length === 0 ? <div className="px-3 py-4 text-sm text-emerald-100/60">Nessun risultato.</div> : null}
                </div>
              </div>
            </div>,
            document.body
