      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex flex-col items-center gap-3">
          <div className="relative">
            <div
              className={`h-32 w-32 rounded-full border border-emerald-100/15 bg-emerald-100/10 overflow-hidden flex items-center justify-center ${logoBusy ? "opacity-70" : ""}`}
            >
              {logoSrc ? (
                <img src={logoSrc} alt="Logo lega" className="h-full w-full object-cover" />
              ) : (
                <div className="text-emerald-100/60 font-semibold text-2xl">{(league.name || "L").slice(0, 2).toUpperCase()}</div>
              )}
            </div>
            ...
            <label
              ...
              className={`absolute -right-2 bottom-3 h-10 w-10 rounded-full border border-emerald-100/15 bg-emerald-950/45 shadow-sm flex items-center justify-center cursor-pointer hover:shadow transition ${logoBusy ? "pointer-events-none opacity-60" : ""}`}
            >
              {logoSrc ? (
                // pencil
            ...
            {logoSrc ? (
              <button
                ...
                title="Rimuovi logo"
                onClick={removeLogo}
                className={`absolute -left-2 bottom-3 h-10 w-10 rounded-full border border-emerald-100/15 bg-emerald-950/45 shadow-sm flex items-center justify-center hover:shadow transition ${logoBusy ? "pointer-events-none opacity-60" : ""}`}
              >
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="text-xs text-emerald-100/60">PNG/JPG/WebP • max ~1.5MB • salvataggio automatico</div>
        </div>
...
function MembersTab() {
  ...
  return (
    <Card>
      ...
      <CardContent>
        {err ? <Alert tone="danger">{err}</Alert> : null}
        <div className="mb-4 flex flex-col gap-2 rounded-xl border border-emerald-100/15 bg-emerald-100/5 p-4 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="font-semibold text-white">Export pronostici</div>
            <div className="text-sm text-emerald-100/60">Scarica una griglia CSV apribile con Excel: utenti sulle righe, match e pronostici torneo sulle colonne.</div>
          </div>
        ...
        <div className="space-y-2">
          {items.map((m) => (
            ...
            <div
              key={m.id}
              className="flex flex-col gap-2 rounded-xl border border-emerald-100/15 p-4 md:flex-row md:items-center md:justify-between"
            >
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {m.status === "APPROVED" ? (
                    m.predictionCheck?.required ? (
                      ...
                      <span className="inline-flex items-center justify-center rounded-full bg-emerald-100/10 px-2 py-0.5 text-xs font-extrabold text-emerald-100/60" title="Nessuna giornata pronosticabile al momento">
                        —
                      </span>
                    )
                  ) : null}
                ...
                {m.status === "APPROVED" ? (
                  m.predictionCheck?.required ? (
                    ...
                    <div className="mt-1 text-xs font-semibold text-emerald-100/60">Nessuna giornata pronosticabile</div>
                  )
                ) : null}
              </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
              ...
              <div className="space-y-3">
                ...
                {(rules as any)?.competitionType === "KNOCKOUT_CUP" ? (
                  <div className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-3 space-y-3">
                    ...
                    <div className="text-xs text-emerald-100/60">Disponibile solo per competizioni configurate dal SuperAdmin come coppa/fasi a eliminazione.</div>
                    <SwitchRow
                  ...
                  <div className="rounded-2xl border border-emerald-100/15 bg-emerald-100/5 p-3 text-xs text-emerald-100/60">
              ...
              <div className="space-y-3">
                ...
                <div className="rounded-2xl border border-emerald-100/15 bg-emerald-100/5 p-3 text-xs text-emerald-100/60">
                  {rules?.scoringMode === "CUMULATIVE" ? (
                    <span>In modalità <b className="text-emerald-50/85">Cumulativo</b> ogni categoria corretta si somma. Se Under/Over 2.5 è attivo, si somma sempre anche lui.</span>
                  ) : rules?.scoringMode === "BEST_ONLY" ? (
                    <span>In modalità <b className="text-emerald-50/85">Solo punteggio più alto</b> viene conteggiata una sola categoria: in caso di parità la priorità è <b className="text-emerald-50/85">Esatto → 1X2 → Somma gol → U/O 2.5</b>.</span>
                  ) : (
                    <span>In modalità <b className="text-emerald-50/85">Mista</b> puoi decidere quali categorie si sommano. Le opzioni sotto governano anche quando <b className="text-emerald-50/85">U/O 2.5</b> si aggiunge a Esatto, 1X2 o Somma gol.</span>
                  )}
                </div>

                {rules?.scoringMode === "MIXED" ? (
                  <div className="rounded-2xl border border-emerald-100/15 p-4 space-y-3">
                    ...
                    {rules?.enableUnderOver25 ? (
                      <div className="mt-2 rounded-xl border border-emerald-100/15 bg-emerald-100/5 p-3 space-y-2">
                        <div className="text-xs font-semibold uppercase tracking-wide text-emerald-100/60">
                      ...
                      <div className="text-xs text-emerald-100/60">(Attiva Under/Over 2.5 per configurare queste opzioni.)</div>
                    )}
                  </div>
                ) : null}
              </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-emerald-100/60">Le modifiche ricalcolano la classifica.</span>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="text-sm text-emerald-50/70 flex items-center gap-2">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="rounded-xl border border-emerald-100/15 p-3">
                  <div className="text-xs text-emerald-100/60">Posizioni a premio</div>
                  <div className="text-sm font-semibold">{prizes.length}</div>
                  <div className="text-xs text-emerald-100/60 mt-1">Aggiungi/rimuovi sotto</div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                {prizes.map((p: any, idx: number) => (
                  <div key={p.position ?? idx} className="flex items-end gap-2">
                    ...
                    <button
                      type="button"
                      className="mb-[6px] h-10 w-10 rounded-xl border border-emerald-100/15 bg-emerald-950/45 hover:shadow-sm transition flex items-center justify-center"
            ...
            <CardContent className="space-y-4">
              ...
              {(() => {
                ...
                if (matchdays.length === 0) {
                  return <div className="text-xs text-emerald-100/60">Nessuna partita disponibile.</div>;
                }
                ...
                return (
                  <div className="space-y-3">
                    {matchdays.map((md) => {
                      ...
                      return (
                        <div key={md} className="rounded-2xl border border-emerald-100/15 p-3">
                          <div className="mb-2 flex items-center justify-between">
                            <div className="text-sm font-semibold text-white">Giornata {md}</div>
                            {current ? <span className="text-xs font-semibold text-amber-200">⭐ Selezionata</span> : <span className="text-xs text-emerald-100/60">—</span>}
                          </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="rounded-2xl border border-emerald-100/15 bg-emerald-100/5 p-4">
                ...
                <ul className="mt-2 list-disc space-y-1 pl-5 text-xs text-emerald-100/60">
              ...
              <Section title="Anticipo lock" hint="Quanto tempo prima del primo match rilevante bloccare i pronostici.">
                ...
                <div className="mt-1 text-xs text-emerald-100/60">Esempio: primo match 20:45 con 30 min → lock dalle 20:15.</div>
              </Section>
              ...
              >
                ...
                />
                <div className="mt-1 text-xs text-emerald-100/60">Suggerimento: imposta la deadline prima della prima giornata.</div>
              </Section>
...
function TieBreakerRow({
  ...
  return (
    <label className="flex items-center justify-between gap-3 text-sm">
      ...
      <select className="rounded-xl border border-emerald-100/15 bg-emerald-950/45 px-3 py-2 text-sm" value={value} onChange={(e) => onChange(e.target.value as any)}>
  ...
  return (
    <span className="relative inline-flex">
      <button
        type="button"
        aria-label="Aiuto"
        className="h-5 w-5 rounded-full border border-emerald-100/15 bg-emerald-950/45 text-emerald-100/60 hover:shadow-sm transition inline-flex items-center justify-center"
        onClick={() => setOpen((v) => !v)}
      >
        ?
      </button>
      ...
      {open ? (
        isMobile ? (
          <div className="fixed inset-0 z-[80]">
            ...
            />
            <div className="absolute bottom-0 left-0 right-0 rounded-t-2xl bg-emerald-950/45 p-4 shadow-2xl">
              <div className="mb-2 flex items-center justify-between">
                <div className="text-base font-semibold text-white">Info</div>
                <button type="button" className="text-sm font-semibold text-emerald-100/60" onClick={() => setOpen(false)}>
                  Chiudi
                </button>
              </div>
              <div className="max-h-[60vh] overflow-auto whitespace-pre-wrap text-sm text-emerald-50/70 pr-1">
                {text}
              </div>
          ...
          <div className="absolute z-30 top-7 right-0 w-72 rounded-xl border border-emerald-100/15 bg-emerald-950/45 p-3 text-xs text-emerald-50/70 shadow-lg">
            <div className="whitespace-pre-wrap">{text}</div>
            <div className="mt-2 flex justify-end">
              <button type="button" className="text-xs text-emerald-100/60 hover:text-emerald-50/70" onClick={() => setOpen(false)}>
                Chiudi
              </button>
            </div>
          </div>
...
function Section({ title, hint, children }: { title: string; hint?: string; children: React.ReactNode }) {
  return (
    <div className="rounded-2xl border border-emerald-100/15 p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-sm font-semibold text-white">{title}</div>
          {hint ? <div className="mt-1 text-xs text-emerald-100/60">{hint}</div> : null}
        </div>
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div className="min-w-0 flex-1">
        <div className="text-sm text-emerald-50/70 flex items-start gap-2">
      ...
      <button
        ...
        className={`relative inline-flex h-6 w-11 shrink-0 items-center rounded-full border transition ${
          disabled
            ? "cursor-not-allowed opacity-60 bg-emerald-100/5 border-emerald-100/15"
            : checked
              ? "bg-emerald-500 border-emerald-400"
              : "bg-slate-200 border-emerald-100/15"
        }`}
      >
        <span
  ...
  return (
    <button
      ...
      className={`text-left rounded-2xl border p-4 transition ${
        checked ? "border-rose-400/45 bg-rose-500/10 shadow-[0_12px_30px_rgba(0,0,0,0.22)]" : "border-white/20 bg-emerald-950/35 hover:border-slate-500 hover:bg-slate-950"
      }`}
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          <div className="text-sm font-semibold text-white">{title}</div>
          <div className="text-xs leading-relaxed text-emerald-50/70">{subtitle}</div>
        </div>
      </div>
    </button>
  );
