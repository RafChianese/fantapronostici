          ...
          {!loading && !error && doc ? (
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                {doc.intro.map((p, idx) => (
                  <p key={idx} className="text-sm text-orange-50/70">
                    {p}
                  </p>
                ))}
              </div>
              ...
              {data?.monetization && (Number(data.monetization.entryFeeCents || 0) > 0 || (data.monetization.prizes || []).length > 0) ? (
                <section className="space-y-3 rounded-2xl border border-amber-100/15 bg-[#07150f]/90 p-4">
                  <h3 className="text-base font-semibold text-white">Quota & Premi</h3>

                  {Number(data.monetization.entryFeeCents || 0) > 0 ? (
                    <div className="flex items-center justify-between rounded-xl bg-white/[0.055] px-3 py-2 ring-1 ring-slate-800/70">
                      <div>
                        <div className="text-xs font-semibold text-orange-50/60">Quota di partecipazione</div>
                  ...
                  {(data.monetization.prizes || []).length > 0 ? (
                    <div className="space-y-2">
                      <div className="text-xs font-semibold text-orange-50/60">Premi</div>
                      <div className="space-y-2">
                        {[...data.monetization.prizes]
                          .sort((a, b) => a.position - b.position)
                          .map((p) => (
                            <div key={p.position} className="flex items-center justify-between rounded-xl border border-amber-100/15 bg-white/[0.055] px-3 py-2">
              ...
              {doc.sections.map((s) => (
                <section key={s.title} className="space-y-2">
                  ...
                  {s.bullets?.length ? (
                    <ul className="list-disc space-y-1 pl-5 text-sm text-orange-50/70">
                      {s.bullets.map((b, idx) => (
                        <li key={idx}>{b}</li>
                      ))}
                    </ul>
