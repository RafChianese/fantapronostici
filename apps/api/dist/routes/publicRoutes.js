...
publicRouter.get("/lock", async (req, res) => {
    ...
    res.json({
        lock: { lockUntil: info.lockUntil, isForceLocked: info.isForceLocked, lockMode: info.lockMode, lockedByTime: info.lockedByTime, isLocked: info.isLocked, autoLock: info.autoLock ?? null },
...
publicRouter.get("/regolamento-config", async (req, res) => {
    ...
    res.json({
        ...
        lock: {
            ...
            autoLock: lockInfo.autoLock ?? null,
        },
    });
});
publicRouter.get("/leaderboard", async (req, res) => {
    ...
    if (sort === "name") {
        ...
        const cmpMetric = (key, dir) => (a, b) => {
            const s = dir === "asc" ? 1 : -1;
            if (a[key] !== b[key])
                return (a[key] - b[key]) * s;
            // secondary: points (same direction for coherence)
            if (a.totalPoints !== b.totalPoints)
                return (a.totalPoints - b.totalPoints) * s;
            return byName(a, b);
        };
        const sorter = sort === "points_asc"
            ? cmpPoints("asc")
            : sort === "points_desc"
                ? cmpPoints("desc")
                : sort === "exact_asc"
