...
function FilterTabs({ filter, setFilter }: { filter: "NEXT" | "ALL" | "LIVE" | "FINISHED"; setFilter: (f: "NEXT" | "ALL" | "LIVE" | "FINISHED") => void }) {
  ...
  return (
    <div className="grid grid-cols-4 overflow-hidden rounded-2xl border border-amber-100/15 bg-white/[0.055] p-1 shadow-[0_10px_30px_rgba(0,0,0,0.25)] backdrop-blur-xl">
      {items.map((it) => (
        <button
          key={it.key}
          type="button"
          onClick={() => setFilter(it.key)}
          className={`rounded-xl px-2 py-2.5 text-xs font-extrabold transition sm:text-sm ${filter === it.key ? "bg-rose-600 text-white shadow-lg" : "text-orange-50/70 hover:bg-white/[0.055]"}`}
        >
          {it.label}
        </button>
      ))}
                    ...
                    return (
                      <div
                        ...
                        className={`rounded-3xl border p-3 transition ${
                          isGold ? "border-yellow-200/40 bg-yellow-300/15 shadow-[0_0_35px_rgba(250,204,21,0.18)] backdrop-blur-xl" : "border-amber-100/15 bg-white/[0.055]"
                        }`}
                      >
                        <div className="flex items-center justify-between gap-3">
