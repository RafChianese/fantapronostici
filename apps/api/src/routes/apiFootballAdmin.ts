  ...
  for (const f of fixtures) {
    ...
    await prisma.match.create({
      data: {
        ...
        homeLogo: (f as any).teams?.home?.crest ?? null,
        awayLogo: (f as any).teams?.away?.crest ?? null,
...
apiFootballAdminRouter.post("/api-football/sync-results", requireAuth, requireSuperAdmin, async (req, res) => {
  ...
  const where: any = { source: "API_FOOTBALL", apiLeagueId: cfg.leagueId, apiSeason: cfg.season };
  if (matchday) where.matchday = matchday;

  const matches = await prisma.match.findMany({ where });
  if (matches.length === 0) return res.json({ ok: true, updated: 0, message: "Nessuna partita da sincronizzare" });

  // Distinct rounds for efficient fetch
  const rounds = Array.from(new Set(matches.map((m) => m.apiRound).filter((x) => !!x))) as string[];
    ...
    if (rounds.length > 0) {
      for (const r of rounds) {
        ...
        for (const f of fixtures) {
          ...
          if (!m) continue;
          const status = mapApiFootballStatus(f.fixture.status?.short) as any;
