...
export async function recomputeMatchdayAwardsForLeague(leagueId, matchday) {
    const rules = await prisma.rule.findUnique({ where: { leagueId } });
    if (!rules || !rules.enableMatchdayAwards)
        return;
...
export async function clearMatchdayAwardsForLeague(leagueId) {
