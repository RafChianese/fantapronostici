...
function PredictionWindowPanel({ config, onSave }: { config: any; onSave: (patch: any) => Promise<void> }) {
  const [start, setStart] = useState(toDatetimeLocalValue(config?.predictionWindowStart));
  const [end, setEnd] = useState(toDatetimeLocalValue(config?.predictionWindowEnd));
  const [saving, setSaving] = useState(false);
  const [ok, setOk] = useState("");
  const [err, setErr] = useState("");

  useEffect(() => {
    setStart(toDatetimeLocalValue(config?.predictionWindowStart));
    setEnd(toDatetimeLocalValue(config?.predictionWindowEnd));
  }, [config?.predictionWindowStart, config?.predictionWindowEnd]);
  ...
  async function saveWindow(clear = false) {
    try {
      ...
      setErr("");
      await onSave({
        predictionWindowStart: clear ? null : datetimeLocalToIso(start),
        predictionWindowEnd: clear ? null : datetimeLocalToIso(end),
      });
      if (clear) {
        setStart("");
        setEnd("");
      }
      setOk(clear ? "Filtro date rimosso." : "Filtro date salvato.");
    } catch (e: any) {
...
function selectedTeamsFromIds(teams: Array<{ id: number; name: string }>, ids: string[]) {
...
function MultiTeamSelect({
  ...
  return (
    <div className="space-y-2 rounded-xl border border-emerald-100/15 bg-emerald-950/45 p-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="text-sm font-semibold text-white">{title}</div>
        <div className="text-xs text-emerald-100/60">Selezionate: {selected.length}</div>
      </div>
      <Input
        label="Cerca squadra"
        value={query}
      ...
      />
      <div className="max-h-48 overflow-auto rounded-xl border border-emerald-100/15 bg-emerald-100/5 p-2">
        {options.length ? options.map((team) => (
          ...
          <div className="px-2 py-2 text-xs text-emerald-100/60">Nessuna squadra disponibile.</div>
        )}
      </div>
      {selected.length ? (
        <Button variant="ghost" onClick={() => onChange([])}>Svuota lista</Button>
      ) : null}
    </div>
  );
}
...
function CompetitionTypePanel({ config, onSave }: { config: any; onSave: (patch: any) => Promise<void> }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {ok ? <Alert tone="success">{ok}</Alert> : null}
      <select
        className="w-full rounded-xl border border-emerald-100/15 bg-emerald-950/45 px-3 py-2 text-sm"
        value={type}
    ...
    <div className="grid gap-6 md:grid-cols-2">
      ...
      <Card className="md:col-span-2">
        ...
        <CardContent className="space-y-4">
          {!compOutcome ? <div className="text-sm text-emerald-100/60">Caricamento…</div> : null}
          {err ? <Alert tone="danger">{err}</Alert> : null}
            ...
            <>
              <div className="grid gap-3 md:grid-cols-2">
                <div className="space-y-2">
                  <div className="text-sm font-semibold text-white">Squadra vincente (globale)</div>
                  <SearchableSelect
                    value={winnerId}
                ...
                <div className="space-y-2 md:col-span-2">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-xs text-emerald-100/60">Usalo solo se vuoi considerare un 2° top scorer (pari merito).</div>
                    </div>
                    {scorer2Id ? (
                ...
                {external?.competitionType === "KNOCKOUT_CUP" ? (
                  <div className="space-y-3 md:col-span-2 rounded-2xl border border-emerald-100/15 bg-emerald-100/5 p-4">
                    <div>
                      <div className="text-sm font-semibold text-white">Squadre qualificate alle fasi a eliminazione</div>
                      <div className="text-xs text-emerald-100/60">Liste globali usate per assegnare i punti ai pronostici del torneo. Puoi selezionare più squadre per ogni fase.</div>
                    </div>
                    <MultiTeamSelect
                      title="Squadre arrivate ai quarti"
                      selected={quarterTeamIds}
                      onChange={setQuarterTeamIds}
                      teams={compOutcome.options?.teams || []}
                    />
                    <MultiTeamSelect
                      title="Squadre arrivate in semifinale"
                      selected={semiTeamIds}
                      onChange={setSemiTeamIds}
                      teams={compOutcome.options?.teams || []}
                    />
                    <MultiTeamSelect
                      title="Squadre arrivate in finale"
                      selected={finalistTeamIds}
                      onChange={setFinalistTeamIds}
                      teams={compOutcome.options?.teams || []}
                    />
                  </div>
                ) : null}
              ...
              <div className="flex items-center gap-2">
                ...
                {compOutcome?.outcome?.resolvedAt ? (
                  <span className="text-xs text-emerald-100/60">Ultimo salvataggio: {new Date(compOutcome.outcome.resolvedAt).toLocaleString("it-IT")}</span>
                ) : (
                  <span className="text-xs text-emerald-100/60">Non ancora impostato.</span>
                )}
              </div>
            </>
          ) : null}
        ...
        <CardContent>
          ...
          <div className="space-y-2">
            {leagues.map((l) => (
              <button
                key={l.id}
                className="w-full rounded-xl border border-emerald-100/15 px-4 py-3 text-left hover:bg-emerald-100/5"
              ...
              >
                ...
                <div className="mt-1 text-xs text-emerald-100/60">Membri: {l._count?.members ?? 0}</div>
              </button>
            ))}
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          {!selected ? (
            <div className="text-sm text-emerald-100/60">Seleziona una lega a sinistra.</div>
          ) : (
...
function MonetizationPanel({
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      <div className="space-y-3">
        <div className="flex items-center justify-between rounded-xl border border-emerald-100/15 p-3">
          <div>
            <div className="font-medium">Ads abilitati</div>
            <div className="text-xs text-emerald-100/60">Se OFF, i pronostici sono visibili senza sblocco</div>
          </div>
          <input
            type="checkbox"
        ...
        <div className="flex items-center justify-between rounded-xl border border-emerald-100/15 p-3">
          <div>
            ...
            <div className="text-xs text-emerald-100/60">Se ON, usa il countdown demo quando Ad Manager non è disponibile</div>
          </div>
          <input
            type="checkbox"
        ...
        <div className="rounded-xl border border-emerald-100/15 p-3">
          ...
          <div className="mt-2 flex items-center gap-2">
            <input
              ...
              className="w-28 rounded-lg border border-emerald-100/15 px-3 py-2"
            />
      ...
      <div className="space-y-3">
        <div className="rounded-xl border border-emerald-100/15 p-3">
          ...
          <div className="mt-2 grid grid-cols-3 gap-2 text-sm">
            <div className="rounded-lg bg-emerald-100/5 p-2">
              <div className="text-xs text-emerald-100/60">Unlock totali</div>
              <div className="font-semibold">{stats?.totalUnlocks ?? 0}</div>
            </div>
            <div className="rounded-lg bg-emerald-100/5 p-2">
              <div className="text-xs text-emerald-100/60">Utenti unici</div>
              <div className="font-semibold">{stats?.uniqueUsers ?? 0}</div>
            </div>
            <div className="rounded-lg bg-emerald-100/5 p-2">
              <div className="text-xs text-emerald-100/60">Media minuti</div>
        ...
        <div className="rounded-xl border border-emerald-100/15 p-3">
          ...
          <div className="mt-2 space-y-2">
            {(stats?.last || []).slice(0, 8).map((r: any) => (
              <div key={r.id} className="flex items-center justify-between rounded-lg bg-emerald-100/5 px-3 py-2">
                <div>
                  <div className="text-sm font-medium">{r.user?.displayName || r.user?.email || r.userId}</div>
                  <div className="text-xs text-emerald-100/60">{new Date(r.createdAt).toLocaleString()}</div>
                </div>
                <Badge>{r.minutes}m</Badge>
              </div>
            ))}
            {(!stats?.last || stats.last.length === 0) ? <div className="text-sm text-emerald-100/60">Nessun dato.</div> : null}
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="tm-page-stack">
      ...
      <div className="flex flex-wrap items-end gap-2">
        <label className="space-y-1 text-sm">
          <div className="text-xs text-emerald-100/60">Season (opz.)</div>
          <input
            type="number"
            className="w-40 rounded-xl border border-emerald-100/15 bg-emerald-950/45 px-3 py-2"
            value={season}
        ...
        <label className="space-y-1 text-sm">
          <div className="text-xs text-emerald-100/60">Area (opz.)</div>
          <input className="w-44 rounded-xl border border-emerald-100/15 bg-emerald-950/45 px-3 py-2" value={area} onChange={(e) => setArea(e.target.value)} placeholder="Italy / Europe" />
        </label>
        <label className="space-y-1 text-sm flex-1 min-w-[200px]">
          <div className="text-xs text-emerald-100/60">Search (opz.)</div>
          <input className="w-full rounded-xl border border-emerald-100/15 bg-emerald-950/45 px-3 py-2" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Serie A, Champions, Worldcup..." />
        </label>
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {comps.slice(0, 12).map((c: any) => {
            ...
            return (
              <button
                key={code}
                className={`rounded-xl border p-3 text-left hover:bg-emerald-100/5 ${chosen?.code === code ? "border-slate-900" : "border-emerald-100/15"}`}
                onClick={() => setChosen({ code, name, area: areaName })}
              >
                <div className="font-medium">{name}</div>
                <div className="text-xs text-emerald-100/60">{areaName} · Code: {code}</div>
              </button>
            );
          })}
        </div>
...
function FootballDataSyncPanel() {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-end gap-2">
        <label className="space-y-1 text-sm">
          <div className="text-xs text-emerald-100/60">Matchday (opz.)</div>
          <input className="w-40 rounded-xl border border-emerald-100/15 bg-emerald-950/45 px-3 py-2" value={matchday} onChange={(e) => setMatchday(e.target.value)} placeholder="es. 1" />
        </label>
        <Button
          disabled={loading}
          onClick={async () => {
      ...
      <div className="text-xs text-emerald-100/60">I risultati vengono sincronizzati automaticamente da football-data.org. La modifica manuale è disabilitata per queste partite.</div>
    </div>
  );
}
  ...
  return (
    <div className="tm-page-stack">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <label className="space-y-1 text-sm">
          <div className="text-xs text-emerald-100/60">Provider</div>
          <select
            className="w-full rounded-xl border border-emerald-100/15 bg-emerald-950/45 px-3 py-2"
        ...
        <label className="space-y-1 text-sm">
          <div className="text-xs text-emerald-100/60">League ID (API-Football)</div>
          <input
            type="number"
            className="w-full rounded-xl border border-emerald-100/15 bg-emerald-950/45 px-3 py-2"
        ...
        <label className="space-y-1 text-sm">
          <div className="text-xs text-emerald-100/60">Season (es. 2025 per 2025/26)</div>
          <input
            type="number"
            className="w-full rounded-xl border border-emerald-100/15 bg-emerald-950/45 px-3 py-2"
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <label className="space-y-1 text-sm md:col-span-2">
          <div className="text-xs text-emerald-100/60">Timezone</div>
          <input
            className="w-full rounded-xl border border-emerald-100/15 bg-emerald-950/45 px-3 py-2"
      ...
      <div className="rounded-xl border border-emerald-100/15 p-3">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          <input
            className="flex-1 rounded-xl border border-emerald-100/15 bg-emerald-950/45 px-3 py-2"
            value={search}
        ...
        {searchResults.length ? (
          <div className="mt-3 grid gap-2 md:grid-cols-2">
            {searchResults.slice(0, 6).map((l: any) => (
              <button
                key={l.id}
                className="rounded-xl border border-emerald-100/15 p-3 text-left hover:bg-emerald-100/5"
                onClick={() => setLocal((x) => ({ ...x, apiFootballLeagueId: l.id }))}
              >
                <div className="font-medium">{l.name}</div>
                <div className="text-xs text-emerald-100/60">{l.country} · ID: {l.id}</div>
              </button>
            ))}
          </div>
        ) : null}
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-xl border border-emerald-100/15 p-3">
        ...
        <div className="text-xs text-emerald-100/60">Code: {league.code} • ID: {league.id}</div>
      </div>
      ...
      {tab === "members" ? (
        <div className="space-y-2">
          {(league.members || []).map((m: any) => (
            <div key={m.id} className="rounded-xl border border-emerald-100/15 p-3">
              <div className="flex items-center justify-between">
                <div>
                  <div className="font-medium">{m.user.displayName}</div>
                  <div className="text-xs text-emerald-100/60">{m.user.email}</div>
                </div>
            ...
            <CardContent>
              ...
              {rules ? (
                <div className="space-y-3">
                  ...
                  <div className="grid gap-2 md:grid-cols-2">
                    <label className="flex items-center gap-2 rounded-xl border border-emerald-100/15 p-3 text-sm">
                      <input
                        type="checkbox"
                  ...
                  <div className="rounded-xl border border-emerald-100/15 p-3">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <div className="text-xs text-emerald-100/60">Bonus punti opzionale</div>
                      </div>
                      <input
                        type="checkbox"
                ...
                <div className="text-sm text-emerald-100/60">Nessuna regola trovata.</div>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              {settings ? (
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between rounded-xl border border-emerald-100/15 p-3">
                    <div>
                      ...
                      <div className="text-xs text-emerald-100/60">Blocca tutto immediatamente</div>
                    </div>
                    <input
                      type="checkbox"
                ...
                <div className="text-sm text-emerald-100/60">Nessuna impostazione trovata.</div>
              )}
            </CardContent>
          </Card>
        </div>
...
function SuperMatchRow({ match, onSaved }: { match: any; onSaved: () => void }) {
  ...
  return (
    <div className="rounded-xl border border-emerald-100/15 bg-emerald-950/45 p-4">
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <div className="font-medium">{match.homeTeam} - {match.awayTeam}</div>
          <div className="text-xs text-emerald-100/60">{new Date(match.kickoffAt).toLocaleString()}</div>
        ...
        <div className="flex flex-wrap items-end gap-2">
          <label className="space-y-1 text-sm">
            <div className="text-xs text-emerald-100/60">Stato</div>
            <select className="rounded-xl border border-emerald-100/15 bg-emerald-950/45 px-3 py-2 text-sm" value={status} onChange={(e) => setStatus(e.target.value)}>
